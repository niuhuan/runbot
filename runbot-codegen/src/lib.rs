...
use syn::{parse_macro_input, spanned::Spanned, FnArg};
...
pub fn message_processor(_args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    if first_param_type != &syn::parse_quote!(Arc<BotContext>) {
        abort!(&first_param.span(), "first parameter must be Arc<BotContext>");
    }
    ...
    if second_param_type != &syn::parse_quote!(Arc<Message>) {
    ...
    emit!(quote::quote! {
        ...
        impl MessageProcessor for #struct_name {
            #asyncness fn process_message(&self, #first_param, #second_param) #return_type #block
        }
